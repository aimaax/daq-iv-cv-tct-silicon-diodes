...
class ChillerCC505:
    ...
    def turn_on_off(self, mode: str, min_runtime: int = 300):
        ...
        try:
            ...
            print("Failed to turn chiller {mode}: {e}")
            # self.connect_chiller()
    ...
    def set_point(self, setpoint_temperature: int):
        ...
        # self.check_chiller_connected()
        try:
            self._send_value(command_type='00', value=setpoint_temperature*100)
        except serial.SerialException as e:
            print(f"Failed to set chiller setpoint: {e}")
            # self.connect_chiller()
    ...
    def reconnect(self):
        """ 
        Method to reconnect Chiller from Reconnect button on Temperature IV CV Control window. 
        """
        self.connect_chiller()